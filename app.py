...
def deep_scan():
    ...
    try:
        ...
        analysis_focus    = data.get('analysis_focus', 'auto')  # 'auto'|'content'|'commerce'|'documentation'|'marketing'
        ...
        # Auto-save to scan library (design knowledge accumulates across sessions)
        scan_id = None
        try:
            from scan_library import save_scan as _lib_save
            scan_id = _lib_save(site_url, cleaned_evidence, analysis_mode=analysis_mode)
            print(f"   📚 Saved to scan library: {scan_id[:8]}…")
        except Exception as lib_err:
            logger.debug(f"Scan library save failed (non-fatal): {lib_err}")
...
@app.route('/api/multi-template/discover', methods=['POST'])
def multi_template_discover_endpoint():
    """
    Phase 1 of two-pass multi-template analysis.

    Fast URL pattern discovery — HTML fetch only, no Playwright.
    Completes in < 15 seconds. Returns URL template clusters.
    The caller then picks which patterns to deep-analyze via Phase 2.

    Request: {"site_url": "https://...", "max_pages": 30, "max_depth": 2}

    Response:
    {
        "patterns": [
            {"template": "/blog/:slug", "count": 12, "examples": [...], "depth": 2, "label": "Blog"}
        ],
        "total_urls_found": 42,
        "discovery_time_s": 4.2,
        "url": "https://..."
    }
    """
    ...
    max_pages = min(int(data.get('max_pages', 30)), 100)
    max_depth = min(int(data.get('max_depth', 2)), 3)

    try:
        from deep_evidence_engine import multi_template_discover
        result = run_async(multi_template_discover(site_url, max_pages=max_pages, max_depth=max_depth))
        return jsonify({'success': True, **result})
    except Exception as e:
        logger.error(f"multi_template_discover failed: {e}", exc_info=True)
...
@app.route('/api/export-design-extract', methods=['POST'])
def export_design_extract():
    """
    Design System Extract — a compact 10-15 line fingerprint card.

    Works for both single-page evidence and smart-nav multi-page evidence.
    Returns JSON with 'card' (Markdown text) and 'data' (structured JSON).

    Accepts: {"evidence": {...}}
    """
    data = request.json or {}
    if 'evidence' not in data:
        # Caller forgot the field entirely — 400 is appropriate
        return jsonify({'error': 'evidence field required'}), 400

    evidence = data.get('evidence', {})
    # Empty dict is allowed — generate_design_system_extract handles it gracefully
    try:
        card, structured = generate_design_system_extract(evidence)
        return jsonify({'success': True, 'card': card, 'data': structured})
    except Exception as e:
        logger.error(f"Design extract failed: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500


def generate_design_system_extract(evidence: dict) -> tuple:
    """
    Produce a 10-15 line Design System Extract card from any evidence dict.

    Handles both single-page evidence (flat keys) and smart-nav evidence
    (nested under page_results).  Returns (markdown_card: str, data: dict).

    The card format is deliberately minimal — it's designed to be copied
    into a Figma file description, a PR comment, or a Claude Code prompt.
    """
    from urllib.parse import urlparse

    # ── Guard: empty evidence ─────────────────────────────────────────────────
    # An empty dict or a dict with no recognisable design keys at all is
    # gracefully declined. Any of the keys below is enough to proceed.
    _DESIGN_KEYS = {
        'typography', 'colors', 'spacing_scale', 'layout', 'motion_tokens',
        'page_results', 'spatial_composition', 'image_strategy', 'css_analytics',
    }
    if not evidence or not (_DESIGN_KEYS & set(evidence.keys())):
        empty_card = (
            "## Design System Extract\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "*Insufficient data — no design metrics found in the provided evidence.*\n"
            "Run a deep scan first: `POST /api/deep-scan`"
        )
        return empty_card, {}

    # ── Resolve evidence to the right structure ───────────────────────────────
    # Smart-nav evidence is nested; single-page is flat.
    is_multi = 'page_results' in evidence
    if is_multi:
        pages = evidence.get('page_results', {})
        home_ev = pages.get('home', next(iter(pages.values()), {}))
        all_pages = list(pages.values())
    else:
        home_ev = evidence
        all_pages = [evidence]

    def _first(ev, *keys, default=None):
        for k in keys:
            v = ev.get(k)
            if v:
                return v
    ...
    def _clean_font(f):
        if not isinstance(f, str):
            return None
        parts = [p.strip().strip('"\'') for p in f.split(',')]
        return next((p for p in parts if p and not p.startswith(('ui-', 'system', '-apple', 'Segoe'))), None)
    clean_fonts = [_clean_font(f) for f in fonts[:4]]
    clean_fonts = [f for f in clean_fonts if f]
    font_chain = ' → '.join(clean_fonts) if clean_fonts else '(not detected)'

    ts = typo.get('type_scale', {})
    if isinstance(ts, dict):
        ratio = ts.get('ratio')
        sizes = ts.get('sizes_px', [])
        heading_sizes = ts.get('heading_sizes_px', [])
        if ratio and sizes:
            # Use the provided ratio field as authoritative.
            # Standard modular scale ratios live between 1.10–1.80 (Minor Third → Golden Ratio).
            # Ratios > 2.0 indicate a display-to-body span (e.g. 60px heading / 16px body = 3.75)
            # rather than a step-by-step progression — label them differently.
            sorted_sizes = sorted(sizes)
            size_range = f"{int(sorted_sizes[0])}–{int(sorted_sizes[-1])}px"
            try:
                r = float(ratio)
            ...
            if r > 2.0:
                type_scale_str = (
                    f"{len(sizes)} sizes ({size_range}, "
                    f"×{ratio} display span)"
                )
            elif r > 0:
                type_scale_str = f"×{ratio} modular ratio, {len(sizes)} steps ({size_range})"
            else:
                type_scale_str = f"{len(sizes)} sizes ({size_range})"
        elif sizes:
            sorted_s = sorted(sizes)
            type_scale_str = f"{len(sizes)} sizes ({int(sorted_s[0])}–{int(sorted_s[-1])}px)"
        else:
            type_scale_str = '(not detected)'
    else:
        type_scale_str = '(not detected)'
    ...
    # Collect unique colors, preferring intentional/primary
    raw_colors = []
    for bucket in ['intentional', 'primary', 'secondary']:
        items = palette.get(bucket, []) if isinstance(palette, dict) else []
        raw_colors.extend(items[:3])
    raw_colors = list(dict.fromkeys(raw_colors))[:5]  # unique, capped

    def _to_hex(c):
        """Convert rgb(r,g,b) → #RRGGBB. Safe against non-string inputs."""
    ...
    hex_colors = [_to_hex(c) for c in raw_colors if c]

    # Role labels — color_roles values are either a string or a list of {value, variable} dicts
    def _extract_role_color(v, avoid_hex=None):
        """
        Extract the most representative color value for a role.

        For roles that can have multiple values (e.g. primary = [off-white, accent-brown]),
        pick the first value that is NOT a near-duplicate of the background color.
        This prevents `primary: #FAF6F0` when background is `#F5F0E8` — both look white
        and one is almost certainly the button-text foreground, not the accent.
        """
        ...
        if isinstance(v, str):
            ...
            candidates = [_get_value(item) for item in v]
        elif isinstance(v, dict):
            candidates = [v.get('value', v.get('color', ''))]

        for c in candidates:
            hex_c = _to_hex(c)
            # Skip empty, non-color, or oklch() values we can't parse
            if not hex_c or not hex_c.startswith('#') or len(hex_c) != 7:
                continue
            # Skip if it's a near-duplicate of the background (distance < 30/765)
            if avoid_hex and _hex_distance(hex_c, avoid_hex) < 30:
                continue
            return hex_c
        return None

    # Extract background first so we can filter primary against it
    background_hex = None
    raw_bg = roles.get('background') if isinstance(roles, dict) else None
    if raw_bg is not None:
        background_hex = _extract_role_color(raw_bg)

    role_parts = []
    for role_name in ['background', 'text', 'accent', 'primary']:
        if not isinstance(roles, dict):
            break
        raw_val = roles.get(role_name)
        if raw_val is None:
            continue
        # For primary: exclude near-duplicates of background
        avoid = background_hex if role_name == 'primary' else None
        val = _extract_role_color(raw_val, avoid_hex=avoid)
        if val:
            role_parts.append(f"{role_name}: {val}")

    # ── Spacing ───────────────────────────────────────────────────────────────
    spacing = home_ev.get('spacing_scale', {})
    base_unit = spacing.get('base_unit', '?')
    scale_vals = spacing.get('scale', spacing.get('values', []))[:6]
    spacing_str = f"{base_unit} base unit ({', '.join(str(v) for v in scale_vals)})" if scale_vals else f"{base_unit} base unit"
    ...
    curves = ep.get('curves', [])
    if durations:
        motion_str = f"{', '.join(str(d) + 'ms' for d in durations[:4])}"
        if curves:
            top_curve = curves[0].get('value', '') if curves else ''
            motion_str += f" / {top_curve}" if top_curve else ''
    else:
        motion_str = '150ms (single duration)' if not durations else '(not detected)'

    # ── Grid system — best extracted from container hierarchy across pages ────
    grid_rows = []
    for page_ev in all_pages:
        meta = page_ev.get('meta_info', {})
        # meta_info.url may be missing on smart-nav pages — also check top-level url key
        url = (meta.get('url') or page_ev.get('url') or page_ev.get('_url', ''))
        path = urlparse(url).path.rstrip('/') or '/' if url else '/'
        label = path.split('/')[-1] or 'home'
        label = label.replace('-', ' ').title() if label != 'home' else 'Homepage'

        spatial = page_ev.get('spatial_composition', {})
        ch = spatial.get('container_hierarchy', {})
        gc = ch.get('grid_containers', {})
        examples = gc.get('examples', [])
        if examples:
            ex = examples[0]
            cols = ex.get('columns', '')
            gap = ex.get('gap', '')
            if cols:
                grid_rows.append((label, cols, gap))
    ...
    for page_ev in all_pages:
        ...
        d = ws.get('content_density_pct')
        if d is not None:
            densities.append(f"{d}%")

    # ── Cross-page consistency ────────────────────────────────────────────────
    dsc = evidence.get('design_system_consistency', {})
    consistency_score = dsc.get('overall_consistency_score')
    consistency_verdict = dsc.get('verdict', '')

    # ── Structured data ───────────────────────────────────────────────────────
    structured = {
        'typography': {'font_chain': font_chain, 'type_scale': type_scale_str},
        'colors': {'hex': hex_colors, 'roles': dict(r.split(': ', 1) for r in role_parts if ': ' in r)},
        'spacing': {'base_unit': base_unit, 'scale': scale_vals},
        'motion': {'raw': motion_str, 'durations_ms': durations, 'curves': len(curves)},
        'grid_system': [{'label': l, 'columns': c, 'gap': g} for l, c, g in grid_rows],
        'progressive_density': densities,
        'consistency': {
            'score': consistency_score,
            'verdict': consistency_verdict,
        } if consistency_score else None,
    }

    # ── Markdown card ─────────────────────────────────────────────────────────
    site_url = home_ev.get('meta_info', {}).get('url', evidence.get('base_url', ''))
    hostname = urlparse(site_url).hostname or site_url

    lines = [
        f"## {hostname} — Design System Extract",
        f"{'━' * min(len(hostname) + 30, 72)}",
        f"**Typography**  {font_chain}",
        f"**Type scale**  {type_scale_str}",
        f"**Colors**      {' '.join(hex_colors) or '(not detected)'}",
    ]
    if role_parts:
        lines.append(f"**Roles**        {' · '.join(role_parts[:3])}")
    lines.append(f"**Spacing**      {spacing_str}")
    lines.append(f"**Motion**       {motion_str}")

    if grid_rows:
        lines.append("**Grid system**")
        for label, cols, gap in grid_rows:
            # Collapse repeated equal columns: "229px 229px 229px 229px" → "229px × 4"
            if len(cols) > 50:
                col_parts = cols.split()
                unique = set(col_parts)
                if len(unique) == 1:
                    cols = f"{col_parts[0]} × {len(col_parts)}"
                else:
                    cols = cols[:50] + '…'
            lines.append(f"  {label:<20} {cols}  (gap: {gap})")
    elif is_multi:
        # Grid data requires scanning content pages, not institutional ones.
        # If smart-nav picked /about or /support, grids won't appear here.
        lines.append("**Grid system**  (not detected — rescan content pages with analysis_focus: content)")

    if densities:
        lines.append(f"**Density**      {' → '.join(densities)}")

    if consistency_score is not None:
        lines.append(f"**Consistency**  {consistency_score}% — {consistency_verdict}")

    if is_multi:
        page_count = len(all_pages)
        lines.append(f"*Extracted from {page_count} pages via Smart Nav*")
...
# ══════════════════════════════════════════════════════════════════════════════
# SCAN LIBRARY — design knowledge persistence across sessions
# ══════════════════════════════════════════════════════════════════════════════

@app.route('/api/library', methods=['GET'])
def library_list():
    """List all saved scans (summary, no full evidence)."""
    domain = request.args.get('domain')
    limit  = min(int(request.args.get('limit', 50)), 200)
    offset = int(request.args.get('offset', 0))
    try:
        from scan_library import list_scans
        scans = list_scans(domain=domain, limit=limit, offset=offset)
...
def library_stats():
    """Return cross-scan aggregate stats — fonts, motion strategies, domains."""
    try:
        from scan_library import get_library_stats
        return jsonify({'success': True, **get_library_stats()})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def library_search():
    ...
    Body: {
        "primary_font": "Playfair%",    // LIKE match
        "motion_strategy": "minimal",
        "spacing_base_unit": "4px",
        "min_confidence": 70,
        "domain": "cinemarchives.com.br"
    }
    """
    query = request.json or {}
    try:
        from scan_library import search_scans
        results = search_scans(query, limit=min(int(query.pop('limit', 20)), 100))
        return jsonify({'success': True, 'scans': results, 'count': len(results)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/library/<scan_id>', methods=['GET'])
def library_get(scan_id):
    """Return a single scan with full evidence."""
    try:
        from scan_library import get_scan
...
@app.route('/api/library/<scan_id>', methods=['DELETE'])
def library_delete(scan_id):
    """Delete a scan and all its findings."""
    try:
        from scan_library import delete_scan
...
@app.route('/api/library/<scan_id>/save', methods=['POST'])
def library_save_existing(scan_id):
    """
    Manually save a scan that was returned from /api/deep-scan but
    not auto-saved (e.g. older scans, or re-saving with new notes).

    Body: {"evidence": {...}, "url": "...", "analysis_mode": "single", "notes": "..."}
    """
    data = request.json or {}
    evidence = data.get('evidence', {})
    url      = data.get('url') or data.get('site_url', '')
    mode     = data.get('analysis_mode', 'single')
    notes    = data.get('notes', '')
    if not evidence or not url:
        return jsonify({'error': 'evidence and url required'}), 400
    try:
        from scan_library import save_scan
        sid = save_scan(url, evidence, analysis_mode=mode, notes=notes, scan_id=scan_id)
...
@app.route('/api/library/findings', methods=['GET'])
def findings_list():
    """List saved findings, optionally filtered by tag or scan_id."""
    tag     = request.args.get('tag')
    scan_id = request.args.get('scan_id')
    limit   = min(int(request.args.get('limit', 50)), 200)
    try:
        from scan_library import list_findings
        findings = list_findings(tag=tag, scan_id=scan_id, limit=limit)
        return jsonify({'success': True, 'findings': findings, 'count': len(findings)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/library/findings', methods=['POST'])
def findings_save():
    """
    Save a finding from the dwell panel.

    Body: {
        "scan_id":       "uuid...",
        "scan_url":      "https://...",
        "section_index": 3,
        "section_data":  {rect, styles, blueprint, ...},
        "label":         "Membership promo layout",
        "tags":          ["zine", "membership", "hero"],
        "notes":         "Great use of contrast for the CTA"
    }
    """
    data = request.json or {}
    ...
    if not scan_id or not section_data:
        return jsonify({'error': 'scan_id and section_data required'}), 400
    try:
        from scan_library import save_finding
        fid = save_finding(
            scan_id=scan_id,
            scan_url=scan_url,
        ...
        return jsonify({'success': True, 'finding_id': fid})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/library/findings/<finding_id>', methods=['DELETE'])
def findings_delete(finding_id):
    try:
        from scan_library import delete_finding
        return jsonify({'success': delete_finding(finding_id)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/library/findings/<finding_id>/tags', methods=['PATCH'])
def findings_update_tags(finding_id):
    """Update tags and optional label on a finding."""
    data  = request.json or {}
    tags  = data.get('tags', [])
    label = data.get('label')
    try:
        from scan_library import update_finding_tags
        return jsonify({'success': update_finding_tags(finding_id, tags, label)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def library_reindex():
    """
    Re-run property indexing on all stored scans.

    Fixes up rows that were saved before:
      - smart-nav flattening was added (font/color/confidence were NULL)
      - motion_strategy was indexed (after motion narrative was wired)
      - any future schema migration

    POST /api/library/reindex
    Response: {"success": true, "updated": N, "errors": N}
    """
    try:
        from scan_library import reindex_all_scans
        result = reindex_all_scans()
        return jsonify({'success': True, **result})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
